...
public class Run {
	...
	public static void main(String[] args) {
		...
		// 실행대기: thread가 실행될 수 있는데 아직 스케줄링이 되지 않아서 실행을 기다리는 상태
		// 일시정지: thread를 실행할 수 없는 상태
		//			   1) waiting: Thread.join(), Object.wait() 메서드에 의해 멈춰진 상태
		//			   2) timed waiting: 지정된 시간동안 쓰레드가 일시정지 되어진 상태. sleep(milliSeconds)
		//			   3) Blocked: 공유자원에 Lock이 걸려 대기하고 있는 경우
		// 중지: thread가 작업을 마치고 중지된 상태
		
		// Thread를 제어하는 메서드
		// start(): 쓰레드를 실행대기 상태로 만들어준다.
		// yield(): 실행중인 쓰레드를 실행대기로 만들어준다.(양보)
		// sleep(): 지정된 시간동안 쓰레드를 일시정지
		// join(): 쓰레드가 종료될 때 까지 해당 쓰레드를 호출한 쓰레드를 일시정지 상태로 변경
		// interrupt(): join() 또는 sleep()에 의해 일시정지 상태가 된 쓰레드를 실행대기로 변경
		// Object.wait(): 쓰레드를 일시정지
		// Object.notify(): Object.wait()에 의해서 일시정지된 쓰레드를 실행대기로 변경
		...
		for (int i = 0; i < 100; i++) {
			if (i == 50) {
				try {
					// main쓰레드를 일시정지 상태로 만듦
					// m2의 쓰레드가 실행이 끝나면 main 쓰레드가 실행대기 상태로 변경
					m2.join();
